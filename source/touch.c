...
int main(int argc, char* argv[])
{
	...
	if (pathLength > 1)
	{
		...
		if (*currentFLC == 0)
		{
			makeFileRoot(pathEnd);
		}
		else
		{
			makeFile(*currentFLC, pathEnd);
		}
	}
	else
	{
...
void makeFile(int FLC, char* pathEnd)
{
	int i;
...
void makeFileRoot(char* pathEnd)
{
	int i;
	unsigned char* buffer;
	buffer = malloc(BYTES_PER_SECTOR * sizeof(unsigned char));
	...
	else
	{
		...
		//removing junk from file extension
		buffer[32 * openSpace + 8] = 0x20;
